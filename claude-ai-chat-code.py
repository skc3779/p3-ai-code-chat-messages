...
def print_banner():
    ...
    # REQ-067-003: 버전 정보를 환경변수에서 로드 (기본값 v1.0.040)
...
{C}║{R}        {Y}{BD} ╚═════╝ ╚══════╝╚═╝  ╚═══╝     ╚═╝  ╚═╝╚═╝{R}                           {C}║{R}
{C}║{R}                                                                              {C}║{R}
{C}║{R}         {B}{BD}>> CLAUDE AI CODE CHAT <<!{R}                                           {C}║{R}
                    ...
                    if parsed.large:
                        try:
                            last_response = LargeContextProcessor(
                                assistant, assistant.file_manager, provider="claude"
                        ...
                        last_response = assistant.chat(
                            question, streaming=streaming_mode, include_context=True,
...
def main_batch(
    ...
    *,
    ...
    try:
        ...
        if context_request is None:
            raise
        print("❌ context 배치 초기화 실패 (claude)", file=sys.stderr)
        return 3
