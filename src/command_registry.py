    ...
    def _register_default_commands(self):
        ...
        self._commands = [
            ...
            CommandInfo('/context',       '파일 컨텍스트와 함께 AI에게 질문',
                        '/context [--large|-l] [-nt] <pattern> [질문]',
                        '--large src/*.py 이 코드 리뷰해줘'),
            ...
            CommandInfo('/agents',        "자율 에이전트 루프 실행 (목표 멀티라인, 루프 중 's' 키 또는 Ctrl+C 로 중단)",
                        '/agents [-ba|--bypassApprovals] [-s <N>] [pattern | [p1,p2,...] | stop]', '[src/*.py, docs/*.md]'),
            CommandInfo('/agents resume', '마지막(또는 지정 인덱스/파일) 에이전트 세션 복원',
                        '/agents resume [N | filename]',          'agent_20260421_123045_tetris.json  또는  2'),
