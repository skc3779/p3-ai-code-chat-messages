...
Claude Code Assistant는 터미널에서 Anthropic Claude API를 활용하여 코드 작성, 리팩토링, 문서화 등을 지원하는 AI 코딩 어시스턴트입니다. 프로젝트 파일 컨텍스트를 자동으로 분석하여 더 정확한 코드 생성을 지원하며, ANSI Art 배너와 Gemini CLI 스타일 TUI를 제공합니다.
...
| 📝 **API 로깅** | 요청/응답 JSON 파일 로깅 (`CLAUDE_AI_LOG_ENABLED`) |
...
| `MAX_TOKENS_CLAUDE` | Claude 토큰 한도 | `150000` |
| `CLAUDE_AI_LOG_ENABLED` | API 요청/응답 JSON 로깅 활성화 | `false` |
...
# 다중 패턴 질의
👤 You: /context [src/*.py, docs/*.md] README.md 파일을 작성해줘
...
```powershell
# PowerShell 스크립트로 빌드
.\build_scripts\build_claude.ps1
```
